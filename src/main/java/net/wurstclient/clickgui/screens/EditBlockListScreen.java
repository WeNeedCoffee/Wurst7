...
public final class EditBlockListScreen extends Screen {
	...
	public void init() {
		...
		blockNameField = new TextFieldWidget(minecraft.textRenderer, width / 2 - 152, height - 55, 150, 18, "");
		children.add(blockNameField);

		addButton(addButton = new ButtonWidget(width / 2 - 2, height - 56, 30, 20, "Add", b -> {
			blockList.add(blockToAdd);
			blockNameField.setText("");
		}));

		addButton(removeButton = new ButtonWidget(width / 2 + 52, height - 56, 100, 20, "Remove Selected", b -> blockList.remove(listGui.selected)));
	...
	public void render(int mouseX, int mouseY, float partialTicks) {
		...
		drawCenteredString(minecraft.textRenderer, blockList.getName() + " (" + listGui.getItemCount() + ")", width / 2, 12, 0xffffff);
		...
		if (blockNameField.getText().isEmpty() && !blockNameField.isFocused())
			drawString(minecraft.textRenderer, "block name or ID", 68, height - 50, 0x808080);
		...
		listGui.renderIconAndGetName(new ItemStack(blockToAdd), 52, height - 52, false);
	...
	private static class ListGui extends ListWidget {
		...
		public ListGui(MinecraftClient mc, EditBlockListScreen screen, List<String> list) {
		...
		protected void renderItem(int index, int x, int y, int var4, int var5, int var6, float partialTicks) {
			...
			fr.draw("ID: " + BlockUtils.getBlockFromName(name), x + 28, y + 18, 0xa0a0a0);
		}
