...
public final class TunnellerHack extends Hack implements UpdateListener, RenderListener {
	private class DigTunnelTask extends Task {
		...
		public void run() {
			...
			WURST.getHax().autoToolHack.equipBestTool(currentBlock, false, true, false);
			breakBlockSimple(currentBlock);

			if (MC.player.abilities.creativeMode || BlockUtils.getHardness(currentBlock) >= 1) {
				progress = 1;
				prevProgress = 1;
				return;
			}
	...
	private class DodgeLiquidTask extends Task {
		...
		public void run() {
			...
			if (!player.equals(pos1)) {
				WURST.getRotationFaker().faceVectorClientIgnorePitch(toVec3d(pos1));
			...
			if (!player.equals(pos2)) {
				WURST.getRotationFaker().faceVectorClientIgnorePitch(toVec3d(pos2));
				forward.setPressed(true);
	...
	private class FillInFloorTask extends Task {
		...
		private boolean equipSolidBlock(BlockPos pos) {
			for (int slot = 0; slot < 9; slot++) {
				...
				if (block instanceof FallingBlock && FallingBlock.canFallThrough(BlockUtils.getState(pos.down()))) {
					continue;
				}
		...
		private BlockPos offsetFloor(BlockPos pos, Vec3i vec) {
			return pos.offset(direction.rotateYCounterclockwise(), vec.getX()).down();
		}
		...
		public void run() {
			...
			Comparator<BlockPos> comparator = Comparator.<BlockPos>comparingDouble(p -> eyes.squaredDistanceTo(new Vec3d(p)));
	...
	private class PlaceTorchTask extends Task {
		...
		public boolean canRun() {
			...
			if (lastTorch != null) {
				nextTorch = lastTorch.offset(direction, size.getSelected().torchDistance);
			}
			...
			return Blocks.TORCH.canPlaceAt(BlockUtils.getState(nextTorch), MC.world, nextTorch);
		}
	...
	private final EnumSetting<TunnelSize> size = new EnumSetting<>("Tunnel size", TunnelSize.values(), TunnelSize.SIZE_3X3);

	private final SliderSetting limit = new SliderSetting("Limit", "Automatically stops once the tunnel\n" + "has reached the given length.\n\n" + "0 = no limit", 0, 0, 1000, 1, v -> v == 0 ? "disabled" : v == 1 ? "1 block" : (int) v + " blocks");
	...
	private boolean breakBlockSimple(BlockPos pos) {
		...
		for (int i = 0; i < sides.length; i++) {
			// check line of sight
			if (MC.world.rayTrace(new RayTraceContext(eyesPos, hitVecs[i], RayTraceContext.ShapeType.COLLIDER, RayTraceContext.FluidHandling.NONE, MC.player)).getType() != HitResult.Type.MISS) {
				continue;
			}
	...
	private BlockPos offset(BlockPos pos, Vec3i vec) {
		return pos.offset(direction.rotateYCounterclockwise(), vec.getX()).up(vec.getY());
	}
	...
	public void onEnable() {
		...
		tasks = new Task[] { new DodgeLiquidTask(), new FillInFloorTask(), new PlaceTorchTask(), new DigTunnelTask(), new WalkForwardTask() };
	...
	public void onUpdate() {
		...
		Hack[] incompatibleHax = { hax.autoToolHack, hax.autoWalkHack, hax.blinkHack, hax.flightHack, hax.nukerHack,
				// TODO:
				// hax.nukerLegitHack,
				// hax.speedNukerHack,
				hax.sneakHack };
	...
	private void updateCyanList() {
		...
		RenderUtils.drawArrow(new Vec3d(direction.getVector()).multiply(0.25), new Vec3d(direction.getVector()).multiply(Math.max(0.5, length)));
