...
public final class ProtectHack extends Hack implements UpdateListener, RenderListener {
	...
	public void onEnable() {
		...
		if (friend == null) {
			...
			friend = stream.min(Comparator.comparingDouble(e -> MC.player.squaredDistanceTo(e))).orElse(null);
		}
	...
	public void onUpdate() {
		...
		if (friend == null || friend.removed || !(friend instanceof LivingEntity) || ((LivingEntity) friend).getHealth() <= 0 || MC.player.getHealth() <= 0) {
			friend = null;
			enemy = null;
			setEnabled(false);
			return;
		...
		if (useAi.isChecked()) {
			...
			// follow target
			WURST.getRotationFaker().faceVectorClient(target.getBoundingBox().getCenter());
			MC.options.keyForward.setPressed(MC.player.distanceTo(target) > (target == friend ? distanceF : distanceE));
		}
