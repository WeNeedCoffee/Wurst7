...
public final class GlideHack extends Hack implements UpdateListener {
	private final SliderSetting fallSpeed = new SliderSetting("Fall speed", 0.125, 0.005, 0.25, 0.005, ValueDisplay.DECIMAL);

	private final SliderSetting moveSpeed = new SliderSetting("Move speed", "Horizontal movement factor.", 1.2, 1, 5, 0.05, ValueDisplay.PERCENTAGE);

	private final SliderSetting minHeight = new SliderSetting("Min height", "Won't glide when you are\n" + "too close to the ground.", 0, 0, 2, 0.01, v -> v == 0 ? "disabled" : ValueDisplay.DECIMAL.getValueString(v));
	...
	public void onUpdate() {
		...
		if (player.onGround || player.isTouchingWater() || player.isInLava() || player.isClimbing() || v.y >= 0)
			return;
