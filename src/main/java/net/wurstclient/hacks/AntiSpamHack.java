...
public final class AntiSpamHack extends Hack implements ChatInputListener {
	...
	public void onReceivedMessage(ChatInputEvent event) {
		...
		List<Text> newLines = Texts.wrapLines(event.getComponent(), maxTextLength, MC.textRenderer, false, false);
		...
		for (int i = chatLines.size() - 1; i >= 0; i--) {
			...
			if (matchingLines <= newLines.size() - 1) {
				...
				if (i > 0 && matchingLines == newLines.size() - 1) {
					String twoLines = oldLine + chatLines.get(i - 1).getText().getString();
