...
public final class RemoteViewHack extends Hack implements UpdateListener, PacketOutputListener {
	private final CheckboxSetting filterPlayers = new CheckboxSetting("Filter players", "Won't view other players.", false);

	private final CheckboxSetting filterSleeping = new CheckboxSetting("Filter sleeping", "Won't view sleeping players.", false);

	private final SliderSetting filterFlying = new SliderSetting("Filter flying", "Won't view players that\n" + "are at least the given\n" + "distance above ground.", 0, 0, 2, 0.05, v -> v == 0 ? "off" : ValueDisplay.DECIMAL.getValueString(v));

	private final CheckboxSetting filterMonsters = new CheckboxSetting("Filter monsters", "Won't view zombies, creepers, etc.", true);

	private final CheckboxSetting filterPigmen = new CheckboxSetting("Filter pigmen", "Won't view zombie pigmen.", true);

	private final CheckboxSetting filterEndermen = new CheckboxSetting("Filter endermen", "Won't view endermen.", true);

	private final CheckboxSetting filterAnimals = new CheckboxSetting("Filter animals", "Won't view pigs, cows, etc.", true);

	private final CheckboxSetting filterBabies = new CheckboxSetting("Filter babies", "Won't view baby pigs,\n" + "baby villagers, etc.", true);

	private final CheckboxSetting filterPets = new CheckboxSetting("Filter pets", "Won't view tamed wolves,\n" + "tamed horses, etc.", true);

	private final CheckboxSetting filterVillagers = new CheckboxSetting("Filter villagers", "Won't view villagers.", true);

	private final CheckboxSetting filterGolems = new CheckboxSetting("Filter golems", "Won't view iron golems,\n" + "snow golems and shulkers.", true);

	private final CheckboxSetting filterInvisible = new CheckboxSetting("Filter invisible", "Won't view invisible entities.", false);
	...
	public void onDisable() {
		...
		// reset entity
		if (entity != null) {
			ChatUtils.message("No longer viewing " + entity.getName().asString() + ".");
	...
	public void onToggledByCommand(String viewName) {
		...
		if (!isEnabled() && viewName != null && !viewName.isEmpty()) {
			entity = StreamSupport.stream(MC.world.getEntities().spliterator(), true).filter(e -> e instanceof LivingEntity).filter(e -> !e.removed && ((LivingEntity) e).getHealth() > 0).filter(e -> e != MC.player).filter(e -> !(e instanceof FakePlayerEntity)).filter(e -> viewName.equalsIgnoreCase(e.getName().asString())).min(Comparator.comparingDouble(e -> MC.player.squaredDistanceTo(e))).orElse(null);
	...
	public void onUpdate() {
		...
		MC.player.resetPosition(entity.getX(), entity.getY() - MC.player.getEyeHeight(MC.player.getPose()) + entity.getEyeHeight(entity.getPose()), entity.getZ());
