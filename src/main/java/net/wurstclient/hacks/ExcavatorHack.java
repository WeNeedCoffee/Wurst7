...
public final class ExcavatorHack extends Hack implements UpdateListener, RenderListener, GUIRenderListener {
	...
	private static class ExcavatorPathFinder extends PathFinder {
		...
		protected boolean checkDone() {
			...
			return done = goal.down(2).equals(current) || goal.up().equals(current) || goal.north().equals(current) || goal.south().equals(current) || goal.west().equals(current) || goal.east().equals(current) || goal.down().north().equals(current) || goal.down().south().equals(current) || goal.down().west().equals(current) || goal.down().east().equals(current);
		}
	}
	...
	private final SliderSetting range = new SliderSetting("Range", 5, 2, 6, 0.05, ValueDisplay.DECIMAL);

	private final EnumSetting<Mode> mode = new EnumSetting<>("Mode", Mode.values(), Mode.FAST);
	...
	private void excavate() {
		...
		// get valid blocks
		Iterable<BlockPos> validBlocks = getValidBlocks(range.getValue(), pos -> area.blocksSet.contains(pos));
		...
		if (pathFinder == null) {
			Comparator<BlockPos> cDistance = Comparator.comparingDouble(pos -> MC.player.squaredDistanceTo(new Vec3d(pos).add(0.5, 0.5, 0.5)));
			Comparator<BlockPos> cAltitude = Comparator.comparingInt(pos -> -pos.getY());
			BlockPos closestBlock = area.blocksList.parallelStream().filter(pClickable).min(cAltitude.thenComparing(cDistance)).get();
	...
	public String getRenderName() {
		...
		if (step == Step.EXCAVATE && area != null) {
			name += " " + (int) ((float) (area.blocksList.size() - area.remainingBlocks) / (float) area.blocksList.size() * 100) + "%";
		}
	...
	private ArrayList<BlockPos> getValidBlocks(double range, Predicate<BlockPos> validator) {
		...
		return BlockUtils.getAllInBox(min, max).stream().filter(pos -> eyesVec.squaredDistanceTo(new Vec3d(pos)) <= rangeSq).filter(BlockUtils::canBeClicked).filter(validator).sorted(Comparator.comparingDouble(pos -> eyesVec.squaredDistanceTo(new Vec3d(pos)))).collect(Collectors.toCollection(() -> new ArrayList<>()));
	}

	private void handlePositionSelection() {
		// continue with next step
		if (step.pos != null && InputUtil.isKeyPressed(MC.getWindow().getHandle(), GLFW.GLFW_KEY_ENTER)) {
	...
	public void onRender(float partialTicks) {
		...
		if (area == null && step == Step.END_POS && step.pos != null) {
			...
			// area box
			GL11.glPushMatrix();
			GL11.glTranslated(preview.minX + offset, preview.minY + offset, preview.minZ + offset);
			GL11.glScaled(preview.sizeX + scale, preview.sizeY + scale, preview.sizeZ + scale);
			GL11.glColor4f(0F, 0F, 0F, 0.5F);
			RenderUtils.drawOutlinedBox();
			GL11.glPopMatrix();
		...
		if (currentBlock != null) {
			// set position
			GL11.glTranslated(currentBlock.getX(), currentBlock.getY(), currentBlock.getZ());
