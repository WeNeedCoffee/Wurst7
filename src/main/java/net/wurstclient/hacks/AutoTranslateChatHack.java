...
public final class AutoTranslateChatHack extends Hack implements ChatInputListener, ChatOutputListener {
	...
	private final EnumSetting<Language> translate_from = new EnumSetting<>("Translate other chat from?", Language.values(), Language.AUTO_DETECT);

	private final EnumSetting<Language> translate_to = new EnumSetting<>("Translate other chat to?", Language.values(), Language.ENGLISH);

	private final EnumSetting<Language> translate_my = new EnumSetting<>("Translate my own chat to?", Language.values(), Language.JAPANESE);
		...
		if (otherchat.isChecked()) {
			new Thread(() -> {
				try {
					...
					if (!message.startsWith(ChatUtils.WURST_PREFIX) && !message.startsWith(String.format("\u00A7a[\u00A7b%s\u00A7a]:\u00a7r ", translate_to.getSelected().name))) {
						String text = StringEscapeUtils.unescapeHtml4(google.translate(message, translate_from.getSelected().value, translate_to.getSelected().value));
						if (text != null) {
	...
	public void onSentMessage(ChatOutputEvent event) {
		...
		if (mychat.isChecked()) {
			if (message.startsWith("/") || message.startsWith(".") || message.startsWith("@"))
				return;
			try {
				String text = StringEscapeUtils.unescapeHtml4(google.translate(message, translate_from.getSelected().value, translate_my.getSelected().value));
				if (text != null) {
