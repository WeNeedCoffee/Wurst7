...
public final class BonemealAuraHack extends Hack implements UpdateListener {
	...
	private ArrayList<BlockPos> getValidBlocks(double range, Predicate<BlockPos> validator) {
		...
		return BlockUtils.getAllInBox(min, max).stream().filter(pos -> eyesVec.squaredDistanceTo(new Vec3d(pos)) <= rangeSq).filter(validator).sorted(Comparator.comparingDouble(pos -> eyesVec.squaredDistanceTo(new Vec3d(pos)))).collect(Collectors.toCollection(() -> new ArrayList<>()));
	}

	private boolean isCorrectBlock(BlockPos pos) {
		Block block = BlockUtils.getBlock(pos);

		if (!(block instanceof Fertilizable) || block instanceof GrassBlock || !((Fertilizable) block).canGrow(MC.world, MC.world.random, pos, BlockUtils.getState(pos)))
			return false;
