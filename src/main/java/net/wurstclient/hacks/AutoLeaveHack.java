...
public final class AutoLeaveHack extends Hack implements UpdateListener {
	private final SliderSetting health = new SliderSetting("Health", "Leaves the server when your health\n" + "reaches this value or falls below it.", 4, 0.5, 9.5, 0.5, v -> ValueDisplay.DECIMAL.getValueString(v) + " hearts");
	...
	public AutoLeaveHack() {
		super("AutoLeave", "Automatically leaves the server\n" + "when your health is low.");
	...
	public void onUpdate() {
		...
		if (MC.isInSingleplayer() && MC.player.networkHandler.getPlayerList().size() == 1)
			return;
		...
		switch (mode.getSelected()) {
			...
			case TELEPORT:
				MC.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.PositionOnly(3.1e7, 100, 3.1e7, false));
				break;

			case SELFHURT:
				MC.player.networkHandler.sendPacket(new PlayerInteractEntityC2SPacket(MC.player));
				break;
		}
