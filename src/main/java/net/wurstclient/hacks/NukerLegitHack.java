...
public final class NukerLegitHack extends Hack implements LeftClickListener, RenderListener, UpdateListener {
	private final SliderSetting range = new SliderSetting("Range", 4.25, 1, 4.25, 0.05, ValueDisplay.DECIMAL);
	...
	public void onLeftClick(LeftClickEvent event) {
		...
		if (pos == null || BlockUtils.getState(pos).getMaterial() == Material.AIR)
			return;
	...
	public void onUpdate() {
		...
		if (mode.getSelected() == Mode.ID && WURST.getHax().nukerHack.getId() == null)
			return;

		currentBlock = null;

		// get valid blocks
		Iterable<BlockPos> validBlocks = getValidBlocks(range.getValue(), mode.getSelected().getValidator(WURST.getHax().nukerHack));
	...
	private boolean breakBlockExtraLegit(BlockPos pos) {
		...
		for (Direction side : Direction.values()) {
			...
			// tick
			if (MC.options.keyAttack.isPressed() && !MC.interactionManager.isBreakingBlock()) {
				MC.options.keyAttack.setPressed(false);
				return true;
			}
	...
	private enum Mode {
		...
		ID("ID", n -> "IDNukerLegit [" + n.getId() + "]", (n, p) -> BlockUtils.getName(p).equals(n.getId())),

		FLAT("Flat", n -> "FlatNukerLegit", (n, p) -> p.getY() >= MC.player.getPos().getY()),

		SMASH("Smash", n -> "SmashNukerLegit", (n, p) -> BlockUtils.getHardness(p) >= 1);
