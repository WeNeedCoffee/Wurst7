...
public final class MultiAuraHack extends Hack implements UpdateListener {
	private final CheckboxSetting useCooldown = new CheckboxSetting("Use cooldown", "Use your weapon's cooldown as the attack speed.\n" + "When checked, the 'Speed' slider will be ignored.", true);

	private final SliderSetting speed = new SliderSetting("Speed", "Attack speed in clicks per second.\n" + "This value will be ignored when\n" + "'Use cooldown' is checked.", 12, 0.1, 20, 0.1, ValueDisplay.DECIMAL);
	...
	public void onUpdate() {
		...
		if (useCooldown.isChecked() ? player.getAttackCooldownProgress(0) < 1 : timer < 1000 / speed.getValue())
			return;

		// get entities
		double rangeSq = Math.pow(range.getValue(), 2);
		Stream<LivingEntity> stream = StreamSupport.stream(world.getEntities().spliterator(), true).filter(e -> e instanceof LivingEntity).map(e -> (LivingEntity) e).filter(e -> !e.removed && e.getHealth() > 0).filter(e -> player.squaredDistanceTo(e) <= rangeSq).filter(e -> e != player).filter(e -> !(e instanceof FakePlayerEntity)).filter(e -> !WURST.getFriends().contains(e.getEntityName()));
