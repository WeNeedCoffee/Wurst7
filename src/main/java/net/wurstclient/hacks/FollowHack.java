...
public final class FollowHack extends Hack implements UpdateListener, RenderListener {
	...
	private final CheckboxSetting filterPlayers = new CheckboxSetting("Filter players", "Won't follow other players.", false);

	private final CheckboxSetting filterSleeping = new CheckboxSetting("Filter sleeping", "Won't follow sleeping players.", false);

	private final SliderSetting filterFlying = new SliderSetting("Filter flying", "Won't follow players that\n" + "are at least the given\n" + "distance above ground.", 0, 0, 2, 0.05, v -> v == 0 ? "off" : ValueDisplay.DECIMAL.getValueString(v));

	private final CheckboxSetting filterMonsters = new CheckboxSetting("Filter monsters", "Won't follow zombies, creepers, etc.", true);

	private final CheckboxSetting filterPigmen = new CheckboxSetting("Filter pigmen", "Won't follow zombie pigmen.", true);

	private final CheckboxSetting filterEndermen = new CheckboxSetting("Filter endermen", "Won't follow endermen.", true);

	private final CheckboxSetting filterAnimals = new CheckboxSetting("Filter animals", "Won't follow pigs, cows, etc.", true);

	private final CheckboxSetting filterBabies = new CheckboxSetting("Filter babies", "Won't follow baby pigs,\n" + "baby villagers, etc.", true);

	private final CheckboxSetting filterPets = new CheckboxSetting("Filter pets", "Won't follow tamed wolves,\n" + "tamed horses, etc.", true);

	private final CheckboxSetting filterVillagers = new CheckboxSetting("Filter villagers", "Won't follow villagers.", true);

	private final CheckboxSetting filterGolems = new CheckboxSetting("Filter golems", "Won't follow iron golems,\n" + "snow golems and shulkers.", true);

	private final CheckboxSetting filterInvisible = new CheckboxSetting("Filter invisible", "Won't follow invisible entities.", false);
	...
	public void onUpdate() {
		...
		if (entity.removed || ((LivingEntity) entity).getHealth() <= 0) {
			entity = StreamSupport.stream(MC.world.getEntities().spliterator(), true).filter(e -> e instanceof LivingEntity).filter(e -> !e.removed && ((LivingEntity) e).getHealth() > 0).filter(e -> e != MC.player).filter(e -> !(e instanceof FakePlayerEntity)).filter(e -> entity.getName().asString().equalsIgnoreCase(e.getName().asString())).min(Comparator.comparingDouble(e -> MC.player.squaredDistanceTo(e))).orElse(null);
		...
		if (useAi.isChecked()) {
			...
			MC.options.keyForward.setPressed(MC.player.squaredDistanceTo(entity.getX(), MC.player.getY(), entity.getZ()) > distanceSq);
		}
	}
