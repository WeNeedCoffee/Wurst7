...
public final class FollowHack extends Hack implements UpdateListener, RenderListener {
	...
	public void onEnable() {
		if (entity == null) {
			entity = StreamSupport.stream(MC.world.getEntities().spliterator(), true).filter(e -> e instanceof LivingEntity).filter(e -> !e.removed && ((LivingEntity) e).getHealth() > 0).filter(e -> e != MC.player).filter(e -> !(e instanceof FakePlayerEntity)).min(Comparator.comparingDouble(e -> MC.player.squaredDistanceTo(e))).orElse(null);
