...
public final class AutoSwordHack extends Hack implements UpdateListener {
	private int oldSlot;
	private int timer;
	private final EnumSetting<Priority> priority = new EnumSetting<>("Priority", Priority.values(), Priority.SPEED);
	private final SliderSetting releaseTime = new SliderSetting("Release time", "Time until AutoSword will switch back from\n" + "the weapon to the previously selected slot.\n\n" + "Only works when \u00a7lSwitch back\u00a7r is checked.", 10, 1, 200, 1, v -> (int) v + " ticks");
	private final CheckboxSetting switchBack = new CheckboxSetting("Switch back", "Switches back to the previously selected slot\n" + "after \u00a7lRelease time\u00a7r has passed.", true);
	...
	private enum Priority {
		SPEED("Speed (swords)"), DAMAGE("Damage (axes)");
