...
public final class FightBotHack extends Hack implements UpdateListener, RenderListener {
	private class EntityPathFinder extends PathFinder {
		...
		protected boolean checkDone() {
			return done = entity.squaredDistanceTo(new Vec3d(current).add(0.5, 0.5, 0.5)) <= Math.pow(distance.getValue(), 2);
		}
	...
	private final SliderSetting range = new SliderSetting("Range", "Attack range (like Killaura)", 4.25, 1, 6, 0.05, ValueDisplay.DECIMAL);

	private final SliderSetting distance = new SliderSetting("Distance", "How closely to follow the target.\n" + "This should be set to a lower value than Range.", 3, 1, 6, 0.05, ValueDisplay.DECIMAL);
	...
	public void onUpdate() {
		...
		if (useAi.isChecked()) {
			...
			// follow entity
			MC.options.keyForward.setPressed(MC.player.distanceTo(entity) > distance.getValueF());
			WURST.getRotationFaker().faceVectorClient(entity.getBoundingBox().getCenter());
		}
