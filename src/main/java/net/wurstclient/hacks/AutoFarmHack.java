...
public final class AutoFarmHack extends Hack implements UpdateListener, RenderListener {
	private final SliderSetting range = new SliderSetting("Range", 5, 1, 60, 0.05, ValueDisplay.DECIMAL);
	...
	public void onEnable() {
		...
		Box box = new Box(1 / 16.0, 1 / 16.0, 1 / 16.0, 15 / 16.0, 15 / 16.0, 15 / 16.0);
	...
	public void onUpdate() {
		...
		if (!WURST.getHax().freecamHack.isEnabled()) // no freecam check
		{
			...
			blocksToReplant = getBlockStream(eyesBlock, blockRange).filter(pos -> plants.containsKey(pos)).filter(pos -> eyesVec.squaredDistanceTo(new Vec3d(pos)) <= rangeSq).filter(pos -> BlockUtils.getState(pos).getMaterial().isReplaceable()).filter(this::canBeReplanted).sorted(Comparator.comparingDouble(pos -> eyesVec.squaredDistanceTo(new Vec3d(pos)))).collect(Collectors.toList());
		}
			...
			if (dist < distSq) {
				...
				if (!isMovingItems) {
					...
					for (int i = 0; i < sides.length; i++) {
						...
						if (MC.world.rayTraceBlock(eyesVec, hitVecs[i], neighbor, neighborShape, neighborState) != null)
							continue;
	...
	private void drawString(String s) {
		...
		int textColor = 0x04 << 24 | (int) (acColor[0] * 256) << 16 | (int) (acColor[1] * 256) << 8 | (int) (acColor[2] * 256);
	...
	public BlockPos findNearestChest() {
		...
		List<BlockPos> chests = getBlockStream(eyesBlock, (int) Math.ceil(range.getValue())).filter(pos -> eyesVec.squaredDistanceTo(new Vec3d(pos)) <= rangeSq).filter(pos -> BlockUtils.canBeClicked(pos)).filter(this::isChest).collect(Collectors.toList());
