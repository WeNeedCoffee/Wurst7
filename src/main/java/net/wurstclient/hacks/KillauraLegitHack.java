...
public final class KillauraLegitHack extends Hack implements UpdateListener, RenderListener {
	...
	private final CheckboxSetting filterInvisible = new CheckboxSetting("Filter invisible", "Won't attack invisible entities.", true);
	...
	private boolean faceEntityClient(LivingEntity entity) {
		...
		return bb.rayTrace(eyesPos, eyesPos.add(lookVec.multiply(range.getValue()))) != null;
	}
	...
	public void onRender(float partialTicks) {
		...
		float p = (target.getMaximumHealth() - target.getHealth()) / target.getMaximumHealth();
		float red = p * 2F;
		float green = 2 - red;

		GL11.glTranslated(target.prevX + (target.getX() - target.prevX) * partialTicks, target.prevY + (target.getY() - target.prevY) * partialTicks, target.prevZ + (target.getZ() - target.prevZ) * partialTicks);
