...
public class ServerFinderScreen extends Screen {
	...
	@Override
	public void init() {
		addButton(searchButton = new ButtonWidget(width / 2 - 100, height / 4 + 96 + 12, 200, 20, "Search", b -> searchOrCancel()));

		addButton(new ButtonWidget(width / 2 - 100, height / 4 + 120 + 12, 200, 20, "Tutorial", b -> Util.getOperatingSystem().open("https://www.wurstclient.net/wiki/Special_Features/Server_Finder/")));

		addButton(new ButtonWidget(width / 2 - 100, height / 4 + 144 + 12, 200, 20, "Back", b -> minecraft.openScreen(prevScreen)));

		ipBox = new TextFieldWidget(font, width / 2 - 100, height / 4 + 34, 200, 20, "");
		ipBox.setMaxLength(200);
		ipBox.setSelected(true);
		children.add(ipBox);

		maxThreadsBox = new TextFieldWidget(font, width / 2 - 32, height / 4 + 58, 26, 12, "");
		...
		WurstClient.INSTANCE.getAnalytics().trackPageView("/multiplayer/server-finder", "Server Finder");
	}
			...
			if (!pingers.get(i).isStillPinging()) {
				...
				if (pingers.get(i).isWorking()) {
					...
					if (!isServerInList(pingers.get(i).getServerIP())) {
						prevScreen.getServerList().add(new ServerInfo("Grief me #" + working, pingers.get(i).getServerIP(), false));
	...
	public void render(int mouseX, int mouseY, float partialTicks) {
		...
		drawCenteredString(font, "This will search for servers with similar IPs", width / 2, 40, 10526880);
		drawCenteredString(font, "to the IP you type into the field below.", width / 2, 50, 10526880);
		drawCenteredString(font, "The servers it finds will be added to your server list.", width / 2, 60, 10526880);

		drawString(font, "Server address:", width / 2 - 100, height / 4 + 24, 10526880);
		ipBox.render(mouseX, mouseY, partialTicks);

		drawString(font, "Max. threads:", width / 2 - 100, height / 4 + 60, 10526880);
		maxThreadsBox.render(mouseX, mouseY, partialTicks);

		drawCenteredString(font, state.toString(), width / 2, height / 4 + 73, 10526880);

		drawString(font, "Checked: " + checked + " / 1792", width / 2 - 100, height / 4 + 84, 10526880);
		drawString(font, "Working: " + working, width / 2 - 100, height / 4 + 94, 10526880);
	...
	enum ServerFinderState {
		NOT_RUNNING(""), SEARCHING("\u00a72Searching..."), RESOLVING("\u00a72Resolving..."), UNKNOWN_HOST("\u00a74Unknown Host!"), CANCELLED("\u00a74Cancelled!"), DONE("\u00a72Done!"), ERROR("\u00a74An error occurred!");
