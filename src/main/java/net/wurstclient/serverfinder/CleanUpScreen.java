...
public class CleanUpScreen extends Screen {
	...
	private void cleanUp() {
		...
		for (int i = prevScreen.getServerList().size() - 1; i >= 0; i--) {
			...
			if (cleanupUnknown && "\u00a74Can\'t resolve hostname".equals(server.label) || cleanupOutdated && server.protocolVersion != SharedConstants.getGameVersion().getProtocolVersion() || cleanupFailed && server.ping != -2L && server.ping < 0L || cleanupGriefMe && server.name.startsWith("Grief me")) {
	...
	public void render(int mouseX, int mouseY, float partialTicks) {
		...
		drawCenteredString(font, "Please select the servers you want to remove:", width / 2, 36, 10526880);
