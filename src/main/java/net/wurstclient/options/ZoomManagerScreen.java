...
public class ZoomManagerScreen extends Screen implements PressAKeyCallback {
	...
	public void init() {
		...
		String zoomKeyName = WurstClient.INSTANCE.getZoomKey().getBoundKey().getName().replace("key.keyboard.", "");

		addButton(new ButtonWidget(width / 2 - 100, height / 4 + 144 - 16, 200, 20, "Back", b -> minecraft.openScreen(prevScreen)));

		addButton(keyButton = new ButtonWidget(width / 2 - 79, height / 4 + 24 - 16, 158, 20, "Zoom Key: " + zoomKeyName, b -> minecraft.openScreen(new PressAKeyScreen(this))));

		addButton(new ButtonWidget(width / 2 - 79, height / 4 + 72 - 16, 50, 20, "More", b -> level.increaseValue()));

		addButton(new ButtonWidget(width / 2 - 25, height / 4 + 72 - 16, 50, 20, "Less", b -> level.decreaseValue()));

		addButton(new ButtonWidget(width / 2 + 29, height / 4 + 72 - 16, 50, 20, "Default", b -> level.setValue(level.getDefaultValue())));
	...
	public void render(int mouseX, int mouseY, float partialTicks) {
		...
		drawString(font, "Zoom Level: " + level.getValueString(), width / 2 - 75, height / 4 + 44, 0xcccccc);
