...
public final class Encryption {
	...
	public Encryption(Path encFolder) {
		KeyPair rsaKeyPair = getRsaKeyPair(encFolder.resolve("wurst_rsa_public.txt"), encFolder.resolve("wurst_rsa_private.txt"));

		SecretKey aesKey = getAesKey(encFolder.resolve("wurst_aes.txt"), rsaKeyPair);

		try {
			encryptCipher = Cipher.getInstance("AES/CFB8/NoPadding");
			encryptCipher.init(Cipher.ENCRYPT_MODE, aesKey, new IvParameterSpec(aesKey.getEncoded()));

			decryptCipher = Cipher.getInstance("AES/CFB8/NoPadding");
			decryptCipher.init(Cipher.DECRYPT_MODE, aesKey, new IvParameterSpec(aesKey.getEncoded()));

		} catch (GeneralSecurityException e) {
			throw new CrashException(CrashReport.create(e, "Creating AES ciphers"));
		}
	}
	...
	private KeyPair createRsaKeys(Path publicFile, Path privateFile) {
		try {
			...
			throw new CrashException(CrashReport.create(e, "Creating RSA keypair"));
		}
	}
