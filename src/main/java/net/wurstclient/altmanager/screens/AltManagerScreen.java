...
public final class AltManagerScreen extends Screen {
	...
	public void init() {
		...
		addButton(new ButtonWidget(width / 2 - 50, height - 52, 100, 20, "Direct Login", b -> minecraft.openScreen(new DirectLoginScreen(this))));

		addButton(new ButtonWidget(width / 2 + 54, height - 52, 100, 20, "Add", b -> minecraft.openScreen(new AddAltScreen(this, altManager))));

		addButton(starButton = new ButtonWidget(width / 2 - 154, height - 28, 75, 20, "Star", b -> pressStar()));

		addButton(editButton = new ButtonWidget(width / 2 - 76, height - 28, 74, 20, "Edit", b -> pressEdit()));

		addButton(deleteButton = new ButtonWidget(width / 2 + 2, height - 28, 74, 20, "Delete", b -> pressDelete()));

		addButton(new ButtonWidget(width / 2 + 80, height - 28, 75, 20, "Cancel", b -> minecraft.openScreen(prevScreen)));

		addButton(new ButtonWidget(8, 8, 100, 20, "Import Alts", b -> pressImportAlts()));
	}
	...
	private void pressDelete() {
		...
		LiteralText message = new LiteralText("\"" + altName + "\" will be lost forever! (A long time!)");
	...
	public void render(int mouseX, int mouseY, float partialTicks) {
		...
		// skin preview
		if (listGui.getSelectedSlot() != -1 && listGui.getSelectedSlot() < altManager.getList().size()) {
			Alt alt = listGui.getSelectedAlt();
		...
		drawCenteredString(font, "Alts: " + altManager.getList().size(), width / 2, 14, 10526880);
		drawCenteredString(font, "premium: " + altManager.getNumPremium() + ", cracked: " + altManager.getNumCracked(), width / 2, 24, 10526880);
