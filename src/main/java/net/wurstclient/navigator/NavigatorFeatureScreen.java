...
public final class NavigatorFeatureScreen extends NavigatorScreen {
	...
	protected void onMouseClick(double x, double y, int button) {
		...
		if (activeButton != null) {
			minecraft.getSoundManager().play(PositionedSoundInstance.master(SoundEvents.UI_BUTTON_CLICK, 1));
			activeButton.press();
			WurstClient.INSTANCE.getNavigator().addPreference(feature.getName());
			return;
		}
	...
	protected void onRender(int mouseX, int mouseY, float partialTicks) {
		...
		drawCenteredString(minecraft.textRenderer, feature.getName(), middleX, 32, 0xffffff);
		...
		drawQuads(bgx1, bgy1, bgx2, Math.max(bgy1, Math.min(bgy2 - (buttons.isEmpty() ? 0 : 24), bgy1 + scroll + window.getY())));
		drawQuads(bgx1, Math.max(bgy1, Math.min(bgy2 - (buttons.isEmpty() ? 0 : 24), bgy1 + scroll + window.getY() + window.getInnerHeight())), bgx2, bgy2);
		drawBoxShadow(bgx1, bgy1, bgx2, bgy2);

		// scissor box
		RenderUtils.scissorBox(bgx1, bgy1, bgx2, bgy2 - (buttons.isEmpty() ? 0 : 24));
		...
		for (ButtonData buttonData : buttonDatas) {
			...
			// text
			GL11.glEnable(GL11.GL_TEXTURE_2D);
			drawCenteredString(minecraft.textRenderer, buttonData.buttonText, (x1 + x2) / 2, y1 + (buttonData.height - 10) / 2 + 1, buttonData.isLocked() ? 0xaaaaaa : buttonData.textColor);
			glDisable(GL_TEXTURE_2D);
			GL11.glEnable(GL11.GL_BLEND);
		}
		...
		for (AbstractButtonWidget button : buttons) {
			...
			// text
			GL11.glEnable(GL11.GL_TEXTURE_2D);
			drawCenteredString(minecraft.textRenderer, button.getMessage(), (x1 + x2) / 2, y1 + 4, 0xffffff);
			GL11.glEnable(GL11.GL_BLEND);
		}
