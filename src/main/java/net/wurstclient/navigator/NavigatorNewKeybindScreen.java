...
public class NavigatorNewKeybindScreen extends NavigatorScreen {
	...
	protected void onRender(int mouseX, int mouseY, float partialTicks) {
		...
		drawCenteredString(minecraft.textRenderer, "New Keybind", middleX, 32, 0xffffff);
		...
		if (!choosingKey) {
			...
			for (PossibleKeybind pkb : possibleKeybinds) {
				...
				// text
				GL11.glEnable(GL11.GL_TEXTURE_2D);
				drawString(minecraft.textRenderer, pkb.getDescription(), x1 + 1, y1 + 1, 0xffffff);
				drawString(minecraft.textRenderer, pkb.getCommand(), x1 + 1, y1 + 1 + minecraft.textRenderer.fontHeight, 0xffffff);
				GL11.glDisable(GL11.GL_TEXTURE_2D);
				GL11.glEnable(GL11.GL_BLEND);
			}
		}
