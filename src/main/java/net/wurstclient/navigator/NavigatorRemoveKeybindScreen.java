...
public class NavigatorRemoveKeybindScreen extends NavigatorScreen {
	...
	protected void onRender(int mouseX, int mouseY, float partialTicks) {
		...
		drawCenteredString(minecraft.textRenderer, "Remove Keybind", middleX, 32, 0xffffff);
		...
		for (Entry<String, PossibleKeybind> entry : existingKeybinds.entrySet()) {
			...
			// text
			GL11.glEnable(GL11.GL_TEXTURE_2D);
			drawString(minecraft.textRenderer, key.replace("key.keyboard.", "") + ": " + keybind.getDescription(), x1 + 1, y1 + 1, 0xffffff);
			drawString(minecraft.textRenderer, keybind.getCommand(), x1 + 1, y1 + 1 + minecraft.textRenderer.fontHeight, 0xffffff);
			GL11.glDisable(GL11.GL_TEXTURE_2D);
			GL11.glEnable(GL11.GL_BLEND);
		}
