...
public final class BlockListSetting extends Setting {
	...
	public BlockListSetting(String name, String description, String... blocks) {
		...
		Arrays.stream(blocks).parallel().map(s -> Registry.BLOCK.get(new Identifier(s))).filter(Objects::nonNull).map(BlockUtils::getName).distinct().sorted().forEachOrdered(s -> blockNames.add(s));
