...
public class SliderSetting extends Setting implements SliderLock {
	public interface ValueDisplay {
		ValueDisplay DECIMAL = v -> Math.round(v * 1e6) / 1e6 + "";

		ValueDisplay INTEGER = v -> (int) v + "";

		ValueDisplay PERCENTAGE = v -> (int) (Math.round(v * 1e8) / 1e6) + "%";
	...
	public SliderSetting(String name, String description, double value, double minimum, double maximum, double increment, ValueDisplay display) {
