...
public final class ItemListSetting extends Setting {
	...
	public ItemListSetting(String name, String description, String... items) {
		...
		Arrays.stream(items).parallel().map(s -> Registry.ITEM.get(new Identifier(s))).filter(Objects::nonNull).map(i -> Registry.ITEM.getId(i).toString()).distinct().sorted().forEachOrdered(s -> itemNames.add(s));
	...
	public void fromJson(JsonElement json) {
		try {
			...
			wson.getAllStrings().parallelStream().map(s -> Registry.ITEM.get(new Identifier(s))).filter(Objects::nonNull).map(i -> Registry.ITEM.getId(i).toString()).distinct().sorted().forEachOrdered(s -> itemNames.add(s));
