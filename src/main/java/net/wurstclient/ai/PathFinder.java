...
public class PathFinder {
	...
	private final boolean invulnerable = WurstClient.MC.player.abilities.creativeMode;
	private final boolean creativeFlying = WurstClient.MC.player.abilities.flying;
	protected final boolean flying = creativeFlying || wurst.getHax().flightHack.isEnabled();
	private final boolean immuneToFallDamage = invulnerable || wurst.getHax().noFallHack.isEnabled();
	...
	private boolean canFallBelow(PathPos pos) {
		...
		for (int i = 0; i <= (fallingAllowed ? 3 : 1); i++) {
			...
			if (prevState.getMaterial() == Material.WATER || prevBlock instanceof LadderBlock || prevBlock instanceof VineBlock || prevBlock instanceof CobwebBlock)
				return true;
	...
	private boolean canMoveSidewaysInMidairAt(BlockPos pos) {
		...
		if (BlockUtils.getState(pos).getMaterial().isLiquid() || blockFeet instanceof LadderBlock || blockFeet instanceof VineBlock || blockFeet instanceof CobwebBlock)
			return true;

		// check head
		Block blockHead = BlockUtils.getBlock(pos.up());
		if (BlockUtils.getState(pos.up()).getMaterial().isLiquid() || blockHead instanceof CobwebBlock)
			return true;
	...
	public boolean isPathStillValid(int index) {
		...
		if (invulnerable != WurstClient.MC.player.abilities.creativeMode || flying != (creativeFlying || wurst.getHax().flightHack.isEnabled()) || immuneToFallDamage != (invulnerable || wurst.getHax().noFallHack.isEnabled())
		// TODO:
		// || noWaterSlowdown !=
		// wurst.getHax().noSlowdownHack.blockWaterSlowness()
				|| jesus != wurst.getHax().jesusHack.isEnabled() || spider != wurst.getHax().spiderHack.isEnabled())
			return false;

		// if index is zero, check if first pos is safe
		if (index == 0) {
			PathPos pos = path.get(0);
			if (!isPassable(pos) || !canFlyAt(pos) && !canGoThrough(pos.down()) && !canSafelyStandOn(pos.down()))
				return false;
		}
