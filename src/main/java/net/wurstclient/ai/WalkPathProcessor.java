...
public class WalkPathProcessor extends PathProcessor {
	...
	public void process() {
		...
		if (MathHelper.wrapDegrees(Math.abs(RotationUtils.getHorizontalAngleToLookVec(new Vec3d(nextPos).add(0.5, 0.5, 0.5)))) > 90)
			return;

		if (WURST.getHax().jesusHack.isEnabled()) {
			// wait for Jesus to swim up
			if (WurstClient.MC.player.getY() < nextPos.getY() && (WurstClient.MC.player.isTouchingWater() || WurstClient.MC.player.isInLava()))
				return;
		...
		if (pos.getX() != nextPos.getX() || pos.getZ() != nextPos.getZ()) {
			...
			if (pos.getY() < nextPos.getY()) {
				...
				if (block instanceof LadderBlock || block instanceof VineBlock) {
					WURST.getRotationFaker().faceVectorClientIgnorePitch(BlockUtils.getBoundingBox(pos).getCenter());
