...
public final class TabGui implements KeyPressListener {
	...
	private void updateSize() {
		width = 64;
		for (Tab tab : tabs) {
			int tabWidth = WurstClient.MC.textRenderer.getStringWidth(tab.name) + 10;
			if (tabWidth > width)
				width = tabWidth;
		}
		height = tabs.size() * 10;
	...
	public void render(float partialTicks) {
		...
		GL11.glScissor((int) (x * factor), (int) ((sr.getScaledHeight() - height - y) * factor), (int) (width * factor), (int) (height * factor));
		...
		if (tabOpened) {
			...
			GL11.glScissor((int) (tabX * factor), (int) ((sr.getScaledHeight() - tab.height - tabY) * factor), (int) (tab.width * factor), (int) (tab.height * factor));
			GL11.glEnable(GL11.GL_SCISSOR_TEST);
			...
			for (int i = 0; i < tab.features.size(); i++) {
				...
				WurstClient.MC.textRenderer.draw(fName, 2, tabTextY, 0xffffffff);
				tabTextY += 10;
			}
			GL11.glEnable(GL11.GL_BLEND);
	...
	private static final class Tab {
		...
		public void updateSize() {
			width = 64;
			for (Feature feature : features) {
				int fWidth = WurstClient.MC.textRenderer.getStringWidth(feature.getName()) + 10;
				if (fWidth > width)
					width = fWidth;
			}
