...
public final class HackListHUD implements UpdateListener {
	...
	public void render(float partialTicks) {
		...
		if (otf.getPosition() == Position.LEFT && WurstClient.INSTANCE.getOtfs().wurstLogoOtf.isVisible())
			posY = 22;
		else
			posY = 2;
		...
		if (WurstClient.INSTANCE.getHax().rainbowUiHack.isEnabled()) {
			...
			textColor = 0x04 << 24 | (int) (acColor[0] * 256) << 16 | (int) (acColor[1] * 256) << 8 | (int) (acColor[2] * 256);
	...
	private void drawWithOffset(HackListEntry e, float partialTicks) {
		...
		float offset = e.offset * partialTicks + e.prevOffset * (1 - partialTicks);
	...
	private static final class HackListEntry implements Comparable<HackListEntry> {
		...
		public int compareTo(HackListEntry o) {
			return hack.getRenderName().compareToIgnoreCase(o.hack.getRenderName());
		}
